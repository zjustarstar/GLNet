...
from model.MSFR import AFEBlock
from model.IEEM import InteractionMEEM
from model.Unet import VGGUNet
from model.MRM import RefUnet
from model.FAM import FeatureAlignmentModule
class Segmenter(nn.Module):
    def __init__(
        ...
        self.mee = InteractionMEEM(in_dim=768,hidden_dim=384)

        self.refnet = RefUnet(in_ch=7,inc_ch=64,num_classes=7)
        self.fam = FeatureAlignmentModule(in_dim=768, adapt_dim=384)
    def forward(self, im):
        ...
        JBfeature = self.encoder.patch_embed(im)
        JBfeature = JBfeature.reshape(B,768,40,40)
        JBfeature = self.mee(JBfeature)
        JBfeature = JBfeature.reshape(B,1600,768)
        ...
        RF_masks,RF_masks_new = self.refnet(masks)

        return masks,RF_masks,RF_masks_new
