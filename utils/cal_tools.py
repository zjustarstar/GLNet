...
class IouCal(object):
    def __init__(self, args):
        ...
        if args.dataset == "cityscapes":
            ...
            self.name = ["BG:", "building:", "car:", "door:", "sky:", "tree:", "window:"]
        elif args.dataset == "ruemonge":
            self.name = ["BG:", "window:", "wall:", "balcony:", "door:", "eaves:", "sky:","shop:"]
        elif args.dataset == "cmp":
            self.name = ["BG:", "facade:", "molding:", "cornice:", "pillar:", "window:", "door:","sill:","blind:","balcony:","shop:","deco:"]
    ...
    def save_results_to_file(self,results, filename="evaluation/results.txt"):
        # Open the file in append mode so new results are added to the file
    ...
    def iou_demo(self):
        ...
        for i in range(len(self.name) - 1):
            STR = STR + self.name[i+1] + str(round(iou[i], 4)) + " "
        print(STR)

        miou = np.nanmean(iou)
        #Format all results into a single string
        # results = (
        #     f"Class IoU: {STR}\n"  # Append the per-class IoU information
        #     f"Overall Accuracy (acc): {round(acc, 4)}\n"
        #     f"Mean Accuracy (acc_cls): {round(acc_cls, 4)}\n"
        #     f"Mean IoU (miou): {round(miou, 4)}\n"
        # )
        # # Save the results to a file
        # self.save_results_to_file(results, "evaluation/results.txt")  # Ensure the filename is correct
        # #
        return round(acc, 4), round(acc_cls, 4), round(miou, 4)
