...
from model.IOUloss import IOUForClasses
    ...
    for data_iter_step, data in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            if args.model_name == "PSPNet":
                ...
                outputs,Unetmask,Refmask = model(inputs)
                # # 初始化损失函数
                ...
                #
                main_loss = criterion(outputs, mask)
                Unetloss = criterion(Unetmask, mask)
                refloss = criterion(Refmask, mask)
                ...
                loss = main_loss+Unetloss+refloss
...
def evaluation_none_training_ours(args, model, val_loader, device):
    ...
    image_folder = r"D:\project\RTFP-improve\data\classall\translated_data_test_hznu\images"  # 传入你的测试图像文件夹路径
...
def evaluation_none_training_生成图像(args, model, val_loader, device):
        model.eval()

        image_folder = r"D:\project\RTFP-improve\data\classall\translated_data_test_hznu_zhengmian_V2\images"  # 传入你的测试图像文件夹路径
        os.makedirs('demo/ori/', exist_ok=True)  # 确保有保存目录
...
def fit_and_fill_polygons(preds, window_class, epsilon_factor=0.02):
    ...
    saved = False
    save_path=r"D:\project\RTFP-master\evaluation\show.jpg"
...
def inference_sliding(args, model, image):
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            with torch.set_grad_enabled(False):
               _, _,padded_prediction = model(img)
    ...
    #window_class = 6
    #preds = fit_and_fill_polygons(preds, window_class, epsilon_factor=0.02)
    return preds, full_probs
...
def inference_sliding_pro(args, model, image):
    ...
    #window_class = 6
    #preds3 = fit_and_fill_polygons(preds3, window_class, epsilon_factor=0.02)
